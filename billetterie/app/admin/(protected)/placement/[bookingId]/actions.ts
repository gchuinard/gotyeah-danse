...
export async function validerPlacement(input: {
  ...
  // placement d'une demande NON réglée (paidAt vide) → on n'envoie pas non plus
  // automatiquement (bouton « Envoyer les billets » + avertissement dans la liste).
  try {
    if (booking.ticketMode === 'email') {
      if (mode === 'emission') {
